...
const allNinjaItems = [
  ...
  {%- for collection in site.collections -%}
    {%- if collection.label != 'posts' and collection.label != 'courses' and collection.label != 'series' -%}
    ...
    {%- for social in site.data.socials -%}
      {%- case social[0] -%}
        {%- when "email_contact" -%}
          ...
          {%- assign email_url = social[1].url | default: "#contact-email" -%}
          {%- capture social_url %}"{{ email_url }}"{% endcapture -%}
        {%- when "acm_id" -%}
        ...
        {%- when "wechat_qr" -%}
          ...
          {%- assign wechat_url = social[1].url | default: "#contact-qr-wechat" -%}
          {%- capture social_url %}"{{ wechat_url }}"{% endcapture -%}

        {%- when "qq_qr" -%}
          {%- assign social_id = "social-qq" -%}
          {%- assign social_title = "QQ" -%}
          {%- assign qq_url = social[1].url | default: "#contact-qr-qq" -%}
          {%- capture social_url %}"{{ qq_url }}"{% endcapture -%}
